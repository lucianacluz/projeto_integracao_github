...
# Treinamentos
## Regressão Linear
salarios_LM <- train(salary ~ yrs.since.phd + yrs.service + Female + Male, data = salarios, method = "lm", trControl = train.control)
summary(salarios_LM) # sumário do modelo linear

## Árvore de Decisão
salarios_RPART <- train(salary ~ yrs.since.phd + yrs.service + Female + Male, data = salarios, method = "rpart", trControl = train.control)
...
salarios_RF <- train(salary ~ yrs.since.phd + yrs.service + Female + Male, data = salarios, method = "cforest", trControl = train.control)
...
salarios_ADA <- train(salary ~ yrs.since.phd + yrs.service + Female + Male, data = salarios, method = "glmboost", trControl = train.control)

plot(salarios_ADA) # evolução do modelo
print(salarios_ADA) # modelo
summary(salarios_ADA) # sumário

melhor_modelo <- resamples(list(LM = salarios_LM, RPART = salarios_RPART, RF = salarios_RF, ADABOOST = salarios_ADA))
melhor_modelo
